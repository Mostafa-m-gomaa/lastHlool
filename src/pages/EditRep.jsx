...
import { createReport, getDuesOverMe, getUsersDues, updateReport } from "@/api/orders";
...
const {data : usersDues , isLoading : usersDuesLoading} = useQuery({
  ...
  const [report,setReport]=useState({})
   useEffect(() => {
    ...
    setReport(report)
    if(report?.companyDues){

      setCash(report?.companyDues);
    }else if(report?.usedCompanyDues){
      setCash(- report?.usedCompanyDues)
    }

  }, []);
const normalizeDeliveredOrders = (orders = []) => {
  return orders.map((item) => ({
    order: item?.order?._id || item.order || "",
    product: item?.order?.product || item.order || "",
    customerName: item?.order?.customersData?.[0]?.customerName || "",
    deliveryReceipt: item?.deliveryReceipt || item.order?.receipt || "",
    deservedSalesManCommission:  item?.order?.salesManCommission || 0,
    deservedSupervisorCommission: item?.order?.supervisorCommission || 0,
    deliveryCommission: item?.order?.deliveryCommission || 0,
    salesMan:  item?.order?.salesPerson?.name || "",
    supervisor:  item?.order?.supervisor?.name || "",
    restCash:  item?.order?.remainingAmount || 0,
    restOrderCost: item?.restOrderCost || [],
  }));
};

const buildUsersCommissionFromOrders = (deliveredOrders, originalDeliveredOrders) => {
  return deliveredOrders.map((order, index) => {
    const fullOrderObj = typeof originalDeliveredOrders?.[index]?.order === "object"
      ? originalDeliveredOrders[index].order
      : null;

    return {
      order: order.order,
      salesManName: fullOrderObj?.salesPerson?.name || order.salesMan || "",
      saledManId: fullOrderObj?.salesPerson?._id || "",

      supervisorName: fullOrderObj?.supervisor?.name || "",
      superVisorId: fullOrderObj?.supervisor?._id || "",

      salesManComm: parseFloat(order.deservedSalesManCommission || 0),
      superVisorComm: parseFloat(order.deservedSupervisorCommission || 0),
      deliveryCommisssion: parseFloat(order.deliveryCommission || 0),
    };
  });
};
...
useEffect(() => {
  if (!report?.outgoings || !report.outgoings.length) return;

  const newCommissions = report.outgoings.map((item) => ({
    userId: item.user._id,
    userName: item.user.name,
    userCommission: item.deservedCommission,
  }));
  ...
  report.outgoings.forEach((item) => {
    const userId = item.user._id;
    const gotten = parseFloat(item.gottenCommission) || 0;
    newInputs[userId] = gotten.toString();
    newPrev[userId] = gotten;
    totalCash += gotten;
  });
...
const newOrdersObj = {
  ...
  const normalizeExtraDeposits = (extraDeposits = []) => {
  return extraDeposits.map((item) => ({
    ...item,
    order: typeof item.order === "object" ? item.order._id : item.order,
  }));
};
const formatDate = (dateInput) => {
  const dateObj = new Date(dateInput); // تحويل حتى لو جالك string
  if (isNaN(dateObj)) return ""; // لو التاريخ بايظ أو فاضي
  ...
  return `${day}-${month}-${year}`;
    ...
    deliveredOrders: normalizeDeliveredOrders(report?.deliveredOrders),
    outgoings: report?.outgoings ||[],
    // categorizedMoney: report?.outgoings ||[],
    burnOuts: report?.burnOuts ||[],
    description: report?.description ||"",
    reportDate: report?.reportDate ||"",
    extraDeposits:normalizeExtraDeposits(report?.extraDeposits) || [],
    userDues: report?.userDues ||[],
    companyDues: report?.companyDues ||"",

  }

const [hasbaCheck,setHasbaCheck]=useState(true)
...
if(hasbaCheck === false){
  ...
  const formatted = formatDate(values.reportDate);
  values.reportDate = formatted;
}
...
if(cashWithMe + cash < 0){
  ...
  const applySetOrderLogic = (orderObj, index, values, setFieldValue, setUsersCommission) => {
  if (!orderObj?._id) return;

  const newCommissionEntry = {
    order: orderObj._id,
    salesManName: orderObj.salesPerson?.name,
    saledManId: orderObj.salesPerson?._id,
    salesManComm: orderObj.salesManCommission,
    supervisorName: orderObj.supervisor?.name,
    superVisorId: orderObj.supervisor?._id,
    superVisorComm: orderObj.supervisorCommission,
    deliveryCommisssion: orderObj.deliveryCommission || 0,
    deliveryManName: orderObj.deliveryMan?.name,
    deliveryManId: orderObj.deliveryMan?._id,
  };
  ...
  updatedDeliveredOrders[index] = {
    ...updatedDeliveredOrders[index],
    customerName: orderObj.customersData?.[0]?.customerName || "",
    deliveryReceipt: orderObj.DeliveryReceipt || "",
    order: orderObj._id || "",
    deservedSalesManCommission: orderObj.salesManCommission || "",
    deservedSupervisorCommission: orderObj.supervisorCommission || "",
    deliveryCommission: orderObj.deliveryCommission || "",
    salesMan: orderObj.salesPerson?.name || "",
    restCash: orderObj.remainingAmount || 0,
  };
...
if (!report || Object.keys(report).length === 0) {
  return <div>جارٍ تحميل التقرير...</div>
}
...
<DialogDemo
  name={values.deliveredOrders[index].product || ""}
  setOrder={(order) => {
                      ...
                      {({ push: pushRestOrderCost, remove: removeRestOrderCost }) => (
                        <div className="flex flex-col gap-4 items-center mx-auto bg-white p-4 w-[90%] rounded-md shadow-2xl ">
                          {values?.extraDeposits?.map((_, restIndex) => (
                            <div key={restIndex} className="flex flex-row-reverse flex-wrap  gap-10 items-center w-[80%] justify-center pt-8 pb-6 rounded-md  bg-gray-200 ">
                          <DialogDemo
                            name={values?.extraDeposits[restIndex]?.order?.product || ""}
                        setOrder={(order) => {        
...
<div className="bg-white flex flex-col gap-10 items-center w-[90%] py-10 px-4 rounded-md shadow-2xl border-2 mx-auto">
  ...
  onClick={() => {
 const rebuiltCommissions = buildUsersCommissionFromOrders(
      values.deliveredOrders,
      report?.deliveredOrders
    );
    setUsersCommission(rebuiltCommissions);
